...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 30, 2022 at 02:54 AM
...
CREATE TABLE `tb_trim_detail` (
  ...
  `id_size` int(200) NOT NULL,
  `id_color` int(200) NOT NULL,
...
INSERT INTO `tb_trim_detail` (`id`, `id_trim`, `id_item`, `id_size`, `id_color`, `qty`, `remark`) VALUES
(46, '2', 47, 1, 4, 1000, 'ok'),
(47, '2', 50, 6, 4, 8000, 'ok'),
(54, '3', 50, 7, 4, 34, 'ccc');
...
INSERT INTO `tb_trim_order` (`id_trim`, `trim_code`, `trim_style`, `trim_destination`, `trim_date`, `trim_status`, `remark`, `user`) VALUES
(2, 'P0122-450525-009 (348M)xx', 'HEATTECH crew neck S/S T-shirt', '(01372F002L) JAPAN', '08/25/2022', 'Request', 'BREAKDOWN			TOTAL\r\nSIZE	COLOR		\r\n	00 WHITE	09 BLACK	\r\nS	5,980	7,000	12980\r\nM	19,020	16,060	35080\r\nL	18,000	17,000	35000\r\nXL	7,000	10,040	17040\r\nTOTAL	50,000	50,100	100,100\r\n', 'rozi@gmail.com'),
(3, 'ccccccc', '444444', 'cccccccccccccccc', '08/25/2022', 'Request', 'cccccccc', 'rozi@gmail.com');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_trimorder_detail`  AS SELECT `tb_trim_detail`.`id` AS `ID`, `tb_trim_detail`.`id_trim` AS `id_trim`, `tb_trim_detail`.`id_item` AS `id_item`, `v_items`.`item_code` AS `item_code`, `v_items`.`remark` AS `item_remark`, `v_items`.`item_description` AS `item_description`, `v_items`.`supplier_name` AS `supplier_name`, `v_items`.`supplier_address` AS `supplier_address`, `v_items`.`supplier_email` AS `supplier_email`, `tb_trim_detail`.`id_size` AS `id_size`, `tb_size`.`size_code` AS `size_code`, `tb_trim_detail`.`id_color` AS `id_color`, `tb_colors`.`color_code` AS `color_code`, `tb_trim_detail`.`qty` AS `qty`, `tb_trim_detail`.`remark` AS `remark` FROM (((`tb_trim_detail` join `tb_size` on(`tb_trim_detail`.`id_size` = `tb_size`.`id_size`)) join `tb_colors` on(`tb_trim_detail`.`id_color` = `tb_colors`.`id_color`)) join `v_items` on(`tb_trim_detail`.`id_item` = `v_items`.`id_item`))  ;
...
--
ALTER TABLE `tb_trim_order`
  MODIFY `id_trim` int(25) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
